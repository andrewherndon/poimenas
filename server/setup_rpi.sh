#!/bin/bash
# Run once on the RPi to set up dnsmasq for Poimenas.
set -e
...
echo -e "# poimenas - unlocked\nserver=$ROUTER_IP\ncache-size=1000" \
    | sudo tee /etc/dnsmasq.d/poimenas.conf > /dev/null

echo "Setting up sudoers rule..."
chmod +x "$SCRIPT_DIR/apply_dns.sh"
echo "$(whoami) ALL=(ALL) NOPASSWD: $SCRIPT_DIR/apply_dns.sh" \
    | sudo tee /etc/sudoers.d/poimenas > /dev/null
sudo chmod 440 /etc/sudoers.d/poimenas
...
echo ""
echo "Done. Set ROUTER_IP in server/.env (default: 192.168.1.1), then restart the poimenas server."
